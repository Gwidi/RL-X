...
class DefaultReward:
    def __init__(self, env):
        ...
        self.foot_velocity_coeff = env.env_config["reward"]["foot_velocity_coeff"] * env.dt
    ...
    def reward_and_info(self, data, mjx_model, internal_state, action, info):
        ...
        # Track linear XY in the world frame. The velocimeter reports values
        # in the IMU frame, so rotate them before comparing them with the
        # global linear velocity command.
        ...
        current_global_linear_velocity = internal_state["imu_orientation_rotation"].apply(current_imu_linear_velocity)
        desired_global_linear_velocity_xy = internal_state["goal_velocities"][:2]
        ...
        # Track yaw velocity in the local IMU frame.
        ...
        yaw_velocity_difference_norm = jnp.square(current_imu_angular_velocity[2] - desired_local_yaw_velocity)
        ...
        feet_positions = data.geom_xpos[self.env.foot_geom_indices]
        ...
        capped_feet_clearance = jnp.minimum(feet_clearance, self.foot_clearance_max_height_m)
        foot_clearance_score = jnp.mean(jnp.square(capped_feet_clearance) * (~feet_floor_contacts))
        ...
        feet_global_linear_velocity_z = data.sensordata[self.env.feet_global_linear_velocity_sensor_adrs_start + 2]
        squared_foot_velocity = jnp.mean(
            feet_global_linear_velocity_xy_norm + jnp.square(feet_global_linear_velocity_z)
        )
        ...
        info[f"reward/foot_velocity"] = foot_velocity_reward
