...
from rl_x.algorithms.ppo_history_window.flax_full_jit.general_properties import GeneralProperties
from rl_x.algorithms.ppo_history_window.flax_full_jit.policy import get_policy
from rl_x.algorithms.ppo_history_window.flax_full_jit.critic import get_critic
...
class PPO_History_Window:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        dummy_window = self.policy.initialize_window(self.nr_envs, self.os_shape[0])

        self.policy_state = TrainState.create(
            apply_fn=self.policy.apply,
            params=self.policy.init(policy_key, env_state.next_observation, dummy_window, method=self.policy.apply_one_step),
            tx=optax.chain(
    ...
    def train(self):
        def jitable_train_function(key, parallel_seed_id):
            ...
            def multi_learning_and_eval_save_iteration(multi_learning_and_eval_save_iteration_carry, multi_learning_iteration_step):
                policy_state, critic_state, env_state, rolling_obs_window, key = multi_learning_and_eval_save_iteration_carry

                def learning_iteration(learning_iteration_carry, learning_iteration_step):
                    policy_state, critic_state, env_state, rolling_obs_window, key = learning_iteration_carry

                    # Acting
                    def single_rollout(single_rollout_carry, _):
                        policy_state, critic_state, env_state, rolling_obs_window, key = single_rollout_carry

                        key, subkey = jax.random.split(key)
                        observation = env_state.next_observation
                        action_mean, action_logstd, next_rolling_obs_window = self.policy.apply(policy_state.params, observation, rolling_obs_window, method=self.policy.apply_one_step)
                        ...
                        next_rolling_obs_window = next_rolling_obs_window * (1.0 - done[:, None, None])
                        ...
                        return (policy_state, critic_state, env_state, next_rolling_obs_window, key), transition

                    rollout_init_policy_carry = rolling_obs_window
                    single_rollout_carry, batch = jax.lax.scan(single_rollout, (policy_state, critic_state, env_state, rolling_obs_window, key), None, self.nr_steps)
                    policy_state, critic_state, env_state, rolling_obs_window, key = single_rollout_carry
                ...
                key, subkey = jax.random.split(key)
                learning_iteration_carry, _ = jax.lax.scan(learning_iteration, (policy_state, critic_state, env_state, rolling_obs_window, subkey), jnp.arange(self.nr_updates_per_multi_learning_iteration))
                policy_state, critic_state, env_state, rolling_obs_window, key = learning_iteration_carry
                ...
                if self.evaluation_active:
                    def single_eval_rollout(single_eval_rollout_carry, _):
                        policy_state, eval_env_state, eval_rolling_obs_window = single_eval_rollout_carry

                        eval_action_mean, _, eval_rolling_obs_window = self.policy.apply(policy_state.params, eval_env_state.next_observation, eval_rolling_obs_window, method=self.policy.apply_one_step)
                        ...
                        eval_rolling_obs_window = eval_rolling_obs_window * (1.0 - eval_done[:, None, None])

                        return (policy_state, eval_env_state, eval_rolling_obs_window), None

                    key, reset_key = jax.random.split(key)
                    reset_keys = jax.random.split(reset_key, self.nr_envs)
                    eval_env_state = self.eval_env.reset(reset_keys, True)
                    eval_rolling_obs_window = self.policy.initialize_window(self.nr_envs, self.os_shape[0])
                    single_eval_rollout_carry, _ = jax.lax.scan(single_eval_rollout, (policy_state, eval_env_state, eval_rolling_obs_window), jnp.arange(self.horizon))
            ...
            jax.lax.scan(multi_learning_and_eval_save_iteration, (policy_state, critic_state, env_state, rolling_obs_window, key), jnp.arange(self.nr_multi_learning_and_eval_save_iterations))
    ...
    def test(self, episodes):
        ...
        @jax.jit
        def rollout(env_state, rolling_obs_window, key):
            # key, subkey = jax.random.split(key)
            action_mean, action_logstd, rolling_obs_window = self.policy.apply(self.policy_state.params, env_state.next_observation, rolling_obs_window, method=self.policy.apply_one_step)
            # action_std = jnp.exp(action_logstd)
            action = action_mean # + action_std * jax.random.normal(subkey, shape=action_mean.shape)
            processed_action = self.get_processed_action(action)
            env_state = self.train_env.step(env_state, processed_action)
            done = env_state.terminated | env_state.truncated
            rolling_obs_window = rolling_obs_window * (1.0 - done[:, None, None])
            return env_state, rolling_obs_window, key
        ...
        rolling_obs_window = self.policy.initialize_window(self.nr_envs, self.os_shape[0])
        while True:
            env_state, rolling_obs_window, self.key = rollout(env_state, rolling_obs_window, self.key)
            if self.render:
