...
from rl_x.algorithms.ppo_history_window.flax.general_properties import GeneralProperties
from rl_x.algorithms.ppo_history_window.flax.policy import get_policy
from rl_x.algorithms.ppo_history_window.flax.critic import get_critic
from rl_x.algorithms.ppo_history_window.flax.batch import Batch

rlx_logger = logging.getLogger("rl_x")


class PPO_History_Window:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.is_training = config.runner.mode == "train"
        self.batch_size = config.environment.nr_envs * config.algorithm.nr_steps
        self.nr_updates = max(1, config.algorithm.total_timesteps // self.batch_size)
        self.nr_minibatches = self.batch_size // self.minibatch_size

        if self.is_training and self.evaluation_frequency % self.batch_size != 0 and self.evaluation_frequency != -1:
            raise ValueError("Evaluation frequency must be a multiple of the number of steps and environments.")
        if self.is_training and self.batch_size % self.minibatch_size != 0:
            raise ValueError("Minibatch size must divide the rollout batch size.")
        if self.is_training and self.minibatch_size % self.nr_steps != 0:
            raise ValueError("History-window PPO minibatch_size must be a multiple of nr_steps.")

        if not self.is_training:
            self.nr_minibatches = max(1, self.nr_minibatches)
        self.nr_minibatch_envs = max(1, self.minibatch_size // self.nr_steps)
        ...
        rolling_obs_window = self.policy.initialize_window(1, self.os_shape[0])

        self.policy_state = TrainState.create(
            apply_fn=self.policy.apply,
            params=self.policy.init(policy_key, state, rolling_obs_window, method=self.policy.apply_one_step),
            tx=optax.chain(
    ...
    def train(self):
        @jax.jit
        def get_action_and_value(policy_state: TrainState, critic_state: TrainState, state: np.ndarray, rolling_obs_window: np.ndarray, key):
            action_mean, action_logstd, next_rolling_obs_window = self.policy.apply(
                policy_state.params,
                state,
                rolling_obs_window,
                method=self.policy.apply_one_step,
            )
        ...
        def update(policy_state: TrainState, critic_state: TrainState,
                   states: np.ndarray, actions: np.ndarray, advantages: np.ndarray, returns: np.ndarray, values: np.ndarray,
                   log_probs: np.ndarray, dones: np.ndarray, init_rolling_obs_window: np.ndarray, key):
            def loss_fn(policy_params, critic_params, state_seq, action_seq, log_prob_seq, return_seq, advantage_seq, done_seq, init_window):
                action_mean, action_logstd = self.policy.apply(
                    policy_params,
                    state_seq,
                    done_seq,
                    init_window,
                    method=self.policy.forward_sequence,
                )
                action_std = jnp.exp(action_logstd)
            ...
            def minibatch_update(carry, minibatch_env_indices):
                ...
                (loss, metrics), (policy_gradients, critic_gradients) = grad_loss_fn(
                    ...
                    init_rolling_obs_window[minibatch_env_indices],
                )
            ...
            (policy_state, critic_state), metrics = jax.lax.scan(minibatch_update, (policy_state, critic_state), batch_env_indices)
        ...
        def get_deterministic_action(policy_state: TrainState, state: np.ndarray, rolling_obs_window: np.ndarray):
            action_mean, _, next_rolling_obs_window = self.policy.apply(
                policy_state.params,
                state,
                rolling_obs_window,
                method=self.policy.apply_one_step,
        ...
        rolling_obs_window = self.policy.initialize_window(self.nr_envs, self.os_shape[0])
        global_step = 0
        nr_updates = 0
        nr_episodes = 0
        steps_metrics = {}
        ...
        while global_step < self.total_timesteps:
            ...
            rollout_init_rolling_obs_window = rolling_obs_window

            dones_this_rollout = 0
            step_info_collection = {}
            for step in range(self.nr_steps):
                processed_action, action, value, log_prob, next_rolling_obs_window, self.key = get_action_and_value(
                    self.policy_state,
                    self.critic_state,
                    state,
                    rolling_obs_window,
                ...
                batch.actions[step] = jax.device_get(action)
                batch.rewards[step] = reward
                batch.values[step] = jax.device_get(value)
                batch.terminations[step] = terminated
                batch.dones[step] = done
                batch.log_probs[step] = jax.device_get(log_prob)

                reset_mask = 1.0 - jnp.asarray(done, dtype=jnp.float32)[:, None, None]
                rolling_obs_window = next_rolling_obs_window * reset_mask
                state = next_state
            ...
            self.policy_state, self.critic_state, optimization_metrics, self.key = update(
                ...
                batch.log_probs,
                batch.dones,
                rollout_init_rolling_obs_window,
                self.key,
            )
            ...
            if global_step % self.evaluation_frequency == 0 and self.evaluation_frequency != -1:
                ...
                eval_nr_envs = eval_state.shape[0]
                eval_rolling_obs_window = self.policy.initialize_window(eval_nr_envs, self.os_shape[0])
                while True:
                    eval_processed_action, eval_next_rolling_obs_window = get_deterministic_action(
                        self.policy_state,
                        eval_state,
                        eval_rolling_obs_window,
                    )
                    ...
                    eval_reset_mask = 1.0 - jnp.asarray(eval_done, dtype=jnp.float32)[:, None, None]
                    eval_rolling_obs_window = eval_next_rolling_obs_window * eval_reset_mask
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = PPO_History_Window(config, train_env, eval_env, run_path, writer)
    ...
    def test(self, episodes):
        ...
        eval_nr_envs = state.shape[0]
        rolling_obs_window = self.policy.initialize_window(eval_nr_envs, self.os_shape[0])
        episode_returns = np.zeros(eval_nr_envs)
        completed_episodes = 0

        while completed_episodes < episodes:
            processed_action, next_rolling_obs_window = get_action(self.policy_state, state, rolling_obs_window)
            ...
            for i, single_done in enumerate(done):
                if single_done:
                    ...
                    if "episode_return" in info:
                        episode_return = self.eval_env.get_final_info_value_at_index(info, "episode_return", i)
                    else:
                        episode_return = episode_returns[i]
                    rlx_logger.info(f"Episode {completed_episodes} - Return: {episode_return}")
