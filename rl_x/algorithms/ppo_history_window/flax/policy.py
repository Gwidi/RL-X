...
class Policy(nn.Module):
    ...
    def window_encode(self, window):
        w_lat = self.window_obs_encode(window)
    ...
    def decode(self, obs_latent, window_latent):
        if self.window_obs_combine_method == "concat":
            ...
            raise ValueError(f"Unknown window_obs_combine_method: {self.window_obs_combine_method}")
        ...
        logstd = jnp.broadcast_to(jnp.squeeze(self.logstd, axis=0), mean.shape)
    ...
    def apply_one_step(self, obs, rolling_window):
        ...
        mean, logstd = self.decode(obs_lat, w_lat)
        next_window = self._update_window(rolling_window, obs)
        return mean, logstd, next_window

    def forward_sequence(self, obs_seq, done_seq, init_window):
        done_prev = jnp.concatenate([jnp.zeros((1,), dtype=jnp.float32), done_seq.astype(jnp.float32)[:-1]], axis=0)
