...
class PPO_Transformer:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.tf_dropout = config.algorithm.tf_dropout
        ...
        if self.tf_dropout > 0.0:
            raise ValueError("Transformer dropout is not supported yet.")
        ...
        dummy_history = self.policy.initialize_history(self.nr_envs, self.os_shape[0])

        self.policy_state = TrainState.create(
            apply_fn=self.policy.apply,
            params=self.policy.init(policy_key, env_state.next_observation, dummy_history, method=self.policy.apply_one_step),
            tx=optax.chain(
    ...
    def train(self):
        def jitable_train_function(key, parallel_seed_id):
            ...
            history = self.policy.initialize_history(self.nr_envs, self.os_shape[0])

            def multi_learning_and_eval_save_iteration(multi_learning_and_eval_save_iteration_carry, multi_learning_iteration_step):
                policy_state, critic_state, env_state, history, key = multi_learning_and_eval_save_iteration_carry

                def learning_iteration(learning_iteration_carry, learning_iteration_step):
                    policy_state, critic_state, env_state, history, key = learning_iteration_carry

                    # Acting
                    def single_rollout(single_rollout_carry, _):
                        policy_state, critic_state, env_state, history, key = single_rollout_carry

                        key, subkey = jax.random.split(key)
                        observation = env_state.next_observation
                        action_mean, action_logstd, next_history = self.policy.apply(policy_state.params, observation, history, method=self.policy.apply_one_step)
                        ...
                        next_history = {
                            "obs": jnp.where(done[:, None, None], jnp.zeros_like(next_history["obs"]), next_history["obs"]),
                            "mask": jnp.where(done[:, None], jnp.zeros_like(next_history["mask"]), next_history["mask"]),
                        }
                        ...
                        return (policy_state, critic_state, env_state, next_history, key), transition

                    rollout_init_history = history
                    single_rollout_carry, batch = jax.lax.scan(single_rollout, (policy_state, critic_state, env_state, history, key), None, self.nr_steps)
                    policy_state, critic_state, env_state, history, key = single_rollout_carry
                    ...
                    vmap_loss_fn = jax.vmap(loss_fn, in_axes=(None, None, 1, 1, 1, 1, 1, 1, {"obs": 0, "mask": 0}), out_axes=0)
                    ...
                    def minibatch_update(carry, minibatch_env_indices):
                        ...
                        minibatch_init_history = {
                            "obs": rollout_init_history["obs"][minibatch_env_indices],
                            "mask": rollout_init_history["mask"][minibatch_env_indices],
                        }
                ...
                key, subkey = jax.random.split(key)
                learning_iteration_carry, _ = jax.lax.scan(learning_iteration, (policy_state, critic_state, env_state, history, subkey), jnp.arange(self.nr_updates_per_multi_learning_iteration))
                policy_state, critic_state, env_state, history, key = learning_iteration_carry
                ...
                if self.evaluation_active:
                    def single_eval_rollout(single_eval_rollout_carry, _):
                        policy_state, eval_env_state, eval_history = single_eval_rollout_carry

                        eval_action_mean, _, eval_history = self.policy.apply(policy_state.params, eval_env_state.next_observation, eval_history, method=self.policy.apply_one_step)
                        ...
                        eval_history = {
                            "obs": jnp.where(eval_done[:, None, None], jnp.zeros_like(eval_history["obs"]), eval_history["obs"]),
                            "mask": jnp.where(eval_done[:, None], jnp.zeros_like(eval_history["mask"]), eval_history["mask"]),
                        }

                        return (policy_state, eval_env_state, eval_history), None

                    key, reset_key = jax.random.split(key)
                    reset_keys = jax.random.split(reset_key, self.nr_envs)
                    eval_env_state = self.eval_env.reset(reset_keys, True)
                    eval_history = self.policy.initialize_history(self.nr_envs, self.os_shape[0])
                    single_eval_rollout_carry, _ = jax.lax.scan(single_eval_rollout, (policy_state, eval_env_state, eval_history), jnp.arange(self.horizon))
            ...
            jax.lax.scan(multi_learning_and_eval_save_iteration, (policy_state, critic_state, env_state, history, key), jnp.arange(self.nr_multi_learning_and_eval_save_iterations))
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = PPO_Transformer(config, train_env, eval_env, run_path, writer)
    ...
    def test(self, episodes):
        ...
        @jax.jit
        def rollout(env_state, history, key):
            # key, subkey = jax.random.split(key)
            action_mean, action_logstd, history = self.policy.apply(self.policy_state.params, env_state.next_observation, history, method=self.policy.apply_one_step)
            # action_std = jnp.exp(action_logstd)
            action = action_mean # + action_std * jax.random.normal(subkey, shape=action_mean.shape)
            processed_action = self.get_processed_action(action)
            env_state = self.train_env.step(env_state, processed_action)
            ...
            history = {
                "obs": jnp.where(done[:, None, None], jnp.zeros_like(history["obs"]), history["obs"]),
                "mask": jnp.where(done[:, None], jnp.zeros_like(history["mask"]), history["mask"]),
            }
            return env_state, history, key
        ...
        history = self.policy.initialize_history(self.nr_envs, self.os_shape[0])
        while True:
            env_state, history, self.key = rollout(env_state, history, self.key)
            if self.render:
