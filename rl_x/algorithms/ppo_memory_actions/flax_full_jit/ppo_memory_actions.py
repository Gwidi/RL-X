...
from rl_x.algorithms.ppo_memory_actions.flax_full_jit.general_properties import GeneralProperties
from rl_x.algorithms.ppo_memory_actions.flax_full_jit.policy import get_policy
from rl_x.algorithms.ppo_memory_actions.flax_full_jit.critic import get_critic
from rl_x.algorithms.flax_full_jit_logging import aggregate_metrics

rlx_logger = logging.getLogger("rl_x")


class PPO_Memory_Actions:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.memory_action_dimension = config.algorithm.memory_action_dimension
        self.memory_action_mean_clip = config.algorithm.memory_action_mean_clip
        ...
        self.env_action_dimension = self.train_env.single_action_space.shape[0]
        self.as_shape = (self.env_action_dimension + self.memory_action_dimension,)
        self.os_shape = (self.train_env.single_observation_space.shape[0] + self.memory_action_dimension,)
        ...
        dummy_memory_action = jnp.zeros((1, self.memory_action_dimension), dtype=jnp.float32)
        dummy_observation = jnp.concatenate([env_state.next_observation, dummy_memory_action], axis=-1)
    ...
    def train(self):
        def jitable_train_function(key, parallel_seed_id):
            ...
            memory_action = jnp.zeros((self.nr_envs, self.memory_action_dimension), dtype=jnp.float32)

            def multi_learning_and_eval_save_iteration(multi_learning_and_eval_save_iteration_carry, multi_learning_iteration_step):
                policy_state, critic_state, env_state, memory_action, key = multi_learning_and_eval_save_iteration_carry

                def learning_iteration(learning_iteration_carry, learning_iteration_step):
                    policy_state, critic_state, env_state, memory_action, key = learning_iteration_carry

                    # Acting
                    def single_rollout(single_rollout_carry, _):
                        policy_state, critic_state, env_state, memory_action, key = single_rollout_carry

                        key, subkey = jax.random.split(key)
                        observation = jnp.concatenate([env_state.next_observation, memory_action], axis=-1)
                        ...
                        env_action, memory_action = jnp.split(action, [self.env_action_dimension], axis=-1)
                        processed_action = self.get_processed_action(env_action)
                        ...
                        memory_action = memory_action / self.memory_action_mean_clip
                        actual_next_observation = jnp.concatenate([env_state.actual_next_observation, memory_action], axis=-1)
                        memory_action = memory_action * (1.0 - done[:, None])

                        transition = (observation, actual_next_observation, action, env_state.reward, value, env_state.terminated, log_prob, env_state.info)
                        ...
                        return (policy_state, critic_state, env_state, memory_action, key), transition
                    ...
                    return (policy_state, critic_state, env_state, memory_action, key), None
                    
                key, subkey = jax.random.split(key)
                learning_iteration_carry, _ = jax.lax.scan(learning_iteration, (policy_state, critic_state, env_state, memory_action, subkey), jnp.arange(self.nr_updates_per_multi_learning_iteration))
                policy_state, critic_state, env_state, memory_action, key = learning_iteration_carry
                ...
                if self.evaluation_active:
                    def single_eval_rollout(single_eval_rollout_carry, _):
                        policy_state, eval_env_state, eval_memory_action = single_eval_rollout_carry

                        eval_observation = jnp.concatenate([eval_env_state.next_observation, eval_memory_action], axis=-1)
                        eval_action_mean, _ = self.policy.apply(policy_state.params, eval_observation)
                        eval_action = eval_action_mean
                        eval_env_action, eval_memory_action = jnp.split(eval_action, [self.env_action_dimension], axis=-1)
                        eval_processed_action = self.get_processed_action(eval_env_action)
                        ...
                        eval_memory_action = eval_memory_action / self.memory_action_mean_clip * (1.0 - eval_done[:, None])

                        return (policy_state, eval_env_state, eval_memory_action), None

                    key, reset_key = jax.random.split(key)
                    reset_keys = jax.random.split(reset_key, self.nr_envs)
                    eval_env_state = self.eval_env.reset(reset_keys, True)
                    eval_memory_action = jnp.zeros((self.nr_envs, self.memory_action_dimension), dtype=jnp.float32)
                    single_eval_rollout_carry, _ = jax.lax.scan(single_eval_rollout, (policy_state, eval_env_state, eval_memory_action), jnp.arange(self.horizon))
            ...
            jax.lax.scan(multi_learning_and_eval_save_iteration, (policy_state, critic_state, env_state, memory_action, key), jnp.arange(self.nr_multi_learning_and_eval_save_iterations))
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = PPO_Memory_Actions(config, train_env, eval_env, run_path, writer)
    ...
    def test(self, episodes):
        ...
        @jax.jit
        def rollout(env_state, memory_action, key):
            # key, subkey = jax.random.split(key)
            observation = jnp.concatenate([env_state.next_observation, memory_action], axis=-1)
            action_mean, action_logstd = self.policy.apply(self.policy_state.params, observation)
            # action_std = jnp.exp(action_logstd)
            action = action_mean # + action_std * jax.random.normal(subkey, shape=action_mean.shape)
            env_action, memory_action = jnp.split(action, [self.env_action_dimension], axis=-1)
            processed_action = self.get_processed_action(env_action)
            ...
            memory_action = memory_action / self.memory_action_mean_clip * (1.0 - done[:, None])
            return env_state, memory_action, key
        ...
        memory_action = jnp.zeros((self.nr_envs, self.memory_action_dimension), dtype=jnp.float32)
        while True:
            env_state, memory_action, self.key = rollout(env_state, memory_action, self.key)
            if self.render:
